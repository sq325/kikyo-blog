...
- 若 event 中显示 `context deadline exceeded`，一般说明该 node 的 kubelet 或下游组件（containerd/runc）有问题，需要登录到 node 节点进一步排查。（需要对容器创建流程有一定了解）
...
linux 进程有两种睡眠状态，S（Interruptible Sleep）和D（Uninterruptible Sleep）。其中S状态的进程正在等待某个事件（如键盘输入）再执行后续操作，此时会被内核挂起，不占用CPU资源，属于正常工作流程中的一环，只要事件到来，进程会被唤醒并执行后续工作。
...
### 磁盘隔离 containerd 和 kubelet 的 volume 能否避免此问题？

containerd 默认路径 `/var/lib/containerd/` 主要功能是把静态的image转换成动态可读写的容器rootfs。

kubelet 默认路径 `/var/lib/kubelet/` 主要功能是把 pod yaml 中定义的各种 volume（emptyDir、configmap 等）挂载进容器的命名空间。
...
一个有效的方法是把 pod 的 IO 流量与 runc 对 rootfs 的操作隔离到不同的物理磁盘，这需要 pod 将 IO 流量导入 PV 而不是 emptyDir。
...
**底层原因：** 由于 emptyDir 挂载在 kubelet 的 LVM 下，pod 对 emptyDir 的高 IO 操作会影响到 runc 对容器环境的初始化。
